...
class OrderFragment : Fragment() {
    ...
    private fun initUi() {
        ...
        orderAdapter = OrderAdapter(activity, listOrder, object : IClickOrderListener {
            ...
            override fun onClickCancerFood(order: Order?) {
                deleteFoodItem(order)
            }
        })
        rcvOrder?.adapter = orderAdapter
    }

    private fun deleteFoodItem(order: Order?) {
        AlertDialog.Builder(activity)
            .setTitle(getString(R.string.msg_delete_title))
            .setMessage(getString(R.string.msg_confirm_delete1))
                ...
                MyApplication[activity!!].getOrderDatabaseReference()
                    ?.child(order!!.id.toString())?.removeValue { _: DatabaseError?, _: DatabaseReference? ->
                        Toast.makeText(activity,
                            getString(R.string.msg_delete_movie_successfully1), Toast.LENGTH_SHORT).show()
                    }
            }
